...
SulpurPipeline::SulpurPipeline(
    SulpurDevice &device, const std::string &vertexShaderFile,
...
SulpurPipeline::~SulpurPipeline()
{
  vkDestroyShaderModule(sulpurDevice.device(), vertexShaderModule, nullptr);
  vkDestroyShaderModule(sulpurDevice.device(), fragmentShaderModule, nullptr);
  vkDestroyPipeline(sulpurDevice.device(), graphicsPipeline, nullptr);
}

void SulpurPipeline::bind(VkCommandBuffer commandBuffer)
{
  vkCmdBindPipeline(
...
void SulpurPipeline::defaultPipelineConfigInfo(PipelineConfigInfo &configInfo)
{
  ...
  configInfo.rasterizationInfo.lineWidth = 1.2f;
  ...
  configInfo.dynamicState.pDynamicStates =
...
void SulpurPipeline::createGraphicsPipeline(
...
)
{
  ...
  if (vkCreateGraphicsPipelines(
          sulpurDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
          &graphicsPipeline
      ) != VK_SUCCESS)
  {
...
void SulpurPipeline::createShaderModule(
...
)
{
  ...
  if (vkCreateShaderModule(
          sulpurDevice.device(), &createInfo, nullptr, shaderModule
      ) != VK_SUCCESS)
  {
