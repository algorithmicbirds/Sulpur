...
void SulpurRenderer::recreateSwapChain() {
    ...
    if (sulpurSwapChain == nullptr) {
        ...
        std::cout << "Recreating Swapchain after resize" << std::endl;
...
VkCommandBuffer SulpurRenderer::beginFrame() {
    assert(!isFrameStarted && "Can't call begin frame while arleady in progress");
...
void SulpurRenderer::beginSwapChainRenderPass(VkCommandBuffer commandBuffer) {
    assert(isFrameStarted && "Cannot call beganRenderPas while frame is not in progress");
    assert(
        commandBuffer == getCurrentCommandBuffer() &&
        "Cannot call beginSwapChainRenderPass on command buffer from another frame"
    );
...
void SulpurRenderer::endSwapChainRenderPass(VkCommandBuffer commandBuffer) {
    ...
    assert(
        commandBuffer == getCurrentCommandBuffer() &&
        "Cannot endSwapChainRenderPass on command buffer from another frame"
    );
